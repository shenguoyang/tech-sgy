...
bool recover_existing_file() {
    ...
    // ═══ 步骤5: 重新索引缺失的数据 ═══
    // 消费端重新同步——根据启用的方案执行对应操作：
    //
    // 方案A: 遍历 committed_seq 之后或 materialized_seq 之前的 Block → 重新物化到 NTFS
    // 方案B: 遍历所有 flags==WRITTEN 的 Block → 重建 Viewer Tool 内存索引
    //         (file_path_hash → block_index 映射)
    // 方案A+B: 两者都执行
    ...
    for (uint32_t i = 0; i < block_count; i++) {
        ...
        if (meta->flags == FLAG_WRITTEN && meta->sequence > materialized_seq) {
            if (config_.enable_approach_a) {
                materialize_block(i, meta);  // 方案A: 写NTFS目录
            }
            if (config_.enable_approach_b) {
                index_block(i, meta);        // 方案B: 重建内存索引
            }
        }
    }

    // ═══ 步骤6: MinIO恢复检查 (可选) ═══
    if (config_.minio_enabled) {
        minio_connect();
        // 遍历所有 BlockMeta，若 data_crc32 校验失败但 MinIO 中存在对应对象
        // → 从 MinIO 下载恢复 Block 数据区
        for (uint32_t i = 0; i < block_count; i++) {
            BlockMeta* meta = &meta_array[i];
            if (meta->flags != FLAG_FREE && meta->data_crc32 != 0) {
                uint64_t block_offset = HEADER_SIZE + BLOCK_META_SIZE * block_count + i * block_size;
                if (!verify_block_crc(i, block_offset)) {
                    // 本地Block损坏，尝试从MinIO恢复
                    if (meta->minio_object_hash != 0 && minio_restore_block(i, meta)) {
                        corrupt_blocks_recovered_++;
                    }
                }
            }
        }
    ...
    // ═══ 步骤7: 启动后台线程 ═══
    if (config_.enable_approach_a) start_materializer_thread();
...
恢复流程:
  1. Initialize() → recover_existing_file()
  2. 步骤6尝试连接MinIO → 失败
  3. CRC校验失败的 Block → 标记 FREE（无法从MinIO恢复）
  4. 已上传MinIO但本地损坏的Block → 恢复暂时不可用
  5. 记录 corrupt_blocks_unrecoverable 计数

恢复结果:
  本地完好数据: ✅ 100%恢复
  MinIO备份但本地损坏: ❌ 暂不可用（待MinIO恢复后手动触发restore命令）
  从未上传MinIO的损坏数据: ❌ 永久丢失

重要: MinIO不可达不影响本地完好数据的恢复。
      只有"本地CRC失败 + 有MinIO备份"的Block受影响，
      这些Block可在MinIO恢复后通过 CLI 命令手动恢复。
```
...
```
  ...
  + 物化目录文件存在     │  ✅ Explorer 可见        │
                         ...
                         ┌─────────────────────────┐
  MinIO-backed 数据      │  MinIO 对象存在          │
  + ETag 匹配            │  ✅ 本地Block损坏时可恢复 │
                         │  ⚠️ 需要MinIO在恢复时可达  │
                         └─────────────────────────┘
```
...
重新物化/索引缺失文件            0-N个文件 × 每个~100ms
MinIO恢复（每个损坏Block）       50-500ms/Block（取决于网络）
─────────────────────────────────────────────────────────
总计（无重新物化/MinIO）         < 12ms
总计（100个Block重新物化）       < 10s
总计（含MinIO恢复10个Block）     < 15s
```

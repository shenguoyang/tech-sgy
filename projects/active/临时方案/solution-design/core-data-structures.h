...
constexpr uint8_t FLAG_WRITTEN                 = 1;  // 已写入, 等待消费
constexpr uint8_t FLAG_MATERIALIZED            = 2;  // 已物化, NTFS目录树可见 (方案A)
constexpr uint8_t FLAG_INDEXED                 = 3;  // 已索引, Viewer Tool可显示 (方案B)
constexpr uint8_t FLAG_MINIO_WARM              = 4;  // 已上传到MinIO温数据桶
constexpr uint8_t FLAG_MINIO_COLD              = 5;  // 已上传到MinIO冷数据桶
constexpr uint8_t FLAG_ARCHIVED_OVERWRITABLE   = 6;  // 已归档, 可被环形覆盖
...
struct BlockMeta {
    ...
    uint64_t minio_object_hash;   // FNV-1a hash of MinIO object key (0 = 未上传)
    uint8_t  reserved[4];         // 预留扩展
};
...
| `file_path` | 相对路径。方案A：物化时拼接为 `data_root + file_path` 创建NTFS文件；方案B：Viewer Tool 据此路径构建虚拟目录树 |
| `reliability` | 三档可靠性标识 |
| `flags` | Block生命周期状态机：<br>方案A：FREE → WRITTEN → MATERIALIZED<br>方案B：FREE → WRITTEN → INDEXED<br>MinIO路径：WRITTEN → MINIO_WARM → MINIO_COLD → ARCHIVED_OVERWRITABLE |
| `data_crc32` | 数据区CRC32，启动时校验数据完整性 |
| `minio_object_hash` | MinIO对象key的FNV-1a hash，0表示未上传 |
...
struct EngineStats {
    ...
    // ═══ 方案B: Viewer Tool状态 ═══
    uint32_t ring_indexed_blocks;        // flags==FLAG_INDEXED 的数量
    bool     viewer_tool_active;         // Viewer Tool是否正在运行

    // ═══ MinIO 状态 (可选) ═══
    uint64_t minio_warm_objects;         // 温数据桶中对象数
    uint64_t minio_cold_objects;         // 冷数据桶中对象数
    uint64_t minio_warm_bytes;           // 温数据桶总字节数
    uint64_t minio_cold_bytes;           // 冷数据桶总字节数
    bool     minio_connected;            // MinIO连接状态
    uint64_t minio_upload_queue_depth;   // 上传队列深度
    uint64_t minio_upload_errors;        // 累计上传失败次数

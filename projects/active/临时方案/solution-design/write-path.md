...
WriteStatus commit_block(uint8_t* aligned_buf, size_t aligned_size,
    ...
    // ═══ 3g: 通知消费端 (Materializer 或 Viewer Tool) ═══
    NotifyConsumer();  // 唤醒 Materializer 线程 或 更新 Viewer Tool 的轮询标记
...
> **MinIO 影响**：当 MinIO 启用时，Block 提交后由独立的 MinIO Client 线程异步上传（非阻塞）。MinIO 上传延迟不贡献到写入关键路径。典型 20MB 文件的上传延迟约 50-200ms（取决于网络），但应用层 `Write()` 调用返回时上传可能尚未开始。
